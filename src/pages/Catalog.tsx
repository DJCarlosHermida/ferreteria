  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="font-display text-sm tracking-[0.3em] text-red">CATÁLOGO</p>
          <h1 className="mt-1 font-display text-4xl uppercase text-cream">Productos</h1>
          <p className="mt-2 max-w-xl text-sm text-muted">
            Filtrá por rubro o buscá por nombre. El precio y el stock se consultan por WhatsApp.
          </p>
        </div>
        <OpenBadge />
      </div>
      ...
      <label className="mt-6 block">
        ...
        <input
          ...
          className="input"
          placeholder="Buscar: taladro, látex, caño..."
          aria-label="Buscar producto"
        />
      </label>

      <div className="sticky top-[4.25rem] z-30 -mx-4 mt-6 overflow-x-auto border-y border-white/10 bg-ink/95 px-4 py-3 backdrop-blur">
        <div className="flex gap-2">
          <FilterChip active={category === "all"} onClick={() => selectCategory("all")}>
            Todo
          </FilterChip>
          {CATEGORIES.map((item) => (
            <FilterChip key={item.id} active={category === item.id} onClick={() => selectCategory(item.id)}>
              {item.label}
            </FilterChip>
          ))}
        </div>
      ...
      {items.length === 0 ? (
        <p className="mt-10 text-center text-muted">No hay productos con esa búsqueda.</p>
      ) : (
