...
export function Footer() {
  ...
  const menuCat = location.pathname === "/catalogo" ? new URLSearchParams(location.search).get("cat") : null
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:grid-cols-3">
        <div>
          <Link to="/" className="inline-block text-cream hover:text-cream-2">
            ...
            <p className="mt-1 text-[10px] font-semibold tracking-[0.28em] text-red">FERRETERÍA</p>
          </Link>
        </div>
        <div className="space-y-3 text-sm text-cream-2">
          ...
          <p className="flex items-start gap-2">
            ...
            Zona: {BRAND.zone}
            {BRAND.delivers ? " · Entrega a domicilio" : ""}
          </p>
        ...
        <nav aria-label="Navegación">
          <p className="font-display text-xs tracking-[0.28em] text-red">RUBROS</p>
          <ul className="mt-3 space-y-2">
            {CATEGORIES.map((item) => (
              <li key={item.id}>
                <Link
                  to={`/catalogo?cat=${item.id}`}
                  className={`text-sm ${menuCat === item.id ? "text-cream" : "text-muted hover:text-cream"}`}
                >
                  {item.label}
                </Link>
              </li>
